...
#CUL3 library sequences
syn23594724 <- synGet("syn23594724")

#CUL3 NTC list
syn23594725 <- synGet("syn23594725")

#CUL3 metascape annotation
syn25617715 <- synGet("syn25617715")
...
CUL3_library <- read_csv(syn23594724$path)
...
save(treatment_joined, control_joined, median_norm, control_norm,
     metadata, CUL3_library, CUL3_synNTC_list, CUL3_GO_GC, median_norm_sgRNA, control_norm_sgRNA, file = "test_comp_data.RData")
...
Colour by p-value, use linked subplots instead `reorder_within` and `scale_y_ordered` from {tidytext} (does not work with the conditional fill)
...
Link this to datatable

```{r}
CUL3_renamed <- CUL3_library %>% 
  rename(sequence = sgrna,
         sgrna = guide)

median_norm_sgRNA_seq <- median_norm_sgRNA %>%
  left_join(select(CUL3_renamed, -gene), by = "sgrna") %>% 
  select(sgrna, Gene, sequence, everything()) %>% 
  filter(str_detect(Gene, "NFK"))

median_norm_sgRNA_shared <- SharedData$new(median_norm_sgRNA_seq,
                                           key = ~sgrna)

p <- ggplot(data = median_norm_sgRNA_shared,
         aes(x = Gene, y = score, color = Gene)) + 
  geom_jitter(aes(group = Gene, sgRNA = sgrna),
              width = 0.1, alpha = 0.6) +
  stat_summary(aes(group = Gene),
               fun = median, geom = "crossbar", 
...
ggp <- ggplotly(p, tooltip = c("sgrna", "y")) %>% 
  highlight(on = "plotly_click",
            persistent = TRUE)

dt <- datatable(median_norm_sgRNA_shared)
...
median_norm_long_ntc <- median_norm_long %>% 
  filter(str_detect(id, "NTC"))

shared_median_norm_long <- SharedData$new(median_norm_long, key = ~ id)
shared_median_norm_long_ntc <- SharedData$new(median_norm_long_ntc, key = ~ id)

# median_norm_long_ntc %>% 
#   filter(score_type == "neg|score") %>% 
#   pull(score) %>% 
#   median() %>% 
#   log10()
# # 0.327; log = 0.485
...
shared_median_norm_long %>%   
  ggplot(aes(x = fct_shuffle(as.factor(id)), y = -log10(score), fill = score_type, text = score)) +
  geom_point(alpha = 0.5, shape = 21, size = 2,
             show.legend = FALSE) +
  geom_point(data = shared_median_norm_long_ntc,
             aes(x = id, y = -log10(score)),
             shape = 21, size = 2,
             fill = "white") +
...
Now without crosstalk, and on the same plot.

Fold change neg|score from median(neg|score) of NTCs
Fold change pos|score from median(pos|score) of NTCs

negative scores on top


```{r}

median_neg_ntc <- median_norm %>% 
  filter(str_detect(id, "NTC")) %>% 
  pull(`neg|score`) %>% 
  median()

median_pos_ntc <- median_norm %>% 
  filter(str_detect(id, "NTC")) %>% 
  pull(`pos|score`) %>% 
  median()
...
p <- median_norm %>% 
  ...
  ggplot(aes(x = fct_shuffle(as.factor(id)), y = change_value, fill = change_type)) +
...
Link the above dot plot to Datatable

```{r}
plotdata <- median_norm %>% 
  select(id, `neg|score`, `pos|score`) %>%
  mutate(neg_log_fold_change = log10(median_neg_ntc / `neg|score`),
         pos_log_fold_change = log10(median_pos_ntc / `pos|score`)) %>%
  # reverse the sign of pos column so it will be on the bottom of the plot
...
plotdata_shared <- SharedData$new(plotdata, key = ~id)


p <-  ggplot(plotdata_shared, aes(x = fct_shuffle(as.factor(id)), y = change_value, fill = change_type)) +
  geom_point(shape = 21, alpha = 0.8, size = 2) +
  geom_point(data = . %>% filter(str_detect(id, "NTC")),
             shape = 21, fill = "white", size = 2, 
             show.legend = FALSE) +
...
ggp <- ggplotly(p, tooltip = c("x", "y")) %>% 
  highlight(on = "plotly_selected",
            off = "plotly_relayout")

dt <- datatable(plotdata_shared)
...
> Dot Plot (after talking to the lab):

Take median of NTCs from `median_norm` file
Plot genes whose neg|score is below neg median; genes whose pos|score is below pos median

**This still seems weird. How to deal with 2 medians (pos and neg), and what about the genes that are below median for both? And the NTCs have a wide range of scores so it's not going to look like the example plot.**

```{r}
ntc_median_neg <- median_norm %>% 
  filter(str_detect(id, "NTC")) %>%
  pull(`neg|score`) %>% 
  median()

ntc_median_pos <- median_norm %>% 
  filter(str_detect(id, "NTC")) %>% 
  pull(`pos|score`) %>% 
  median()

neg_genes <- median_norm %>% 
  filter(`neg|score` <= ntc_median_neg) %>% 
  select(id, `neg|score`) %>% 
  pivot_longer(cols = -id,
               names_to = "score_type",
               values_to = "score")

pos_genes <- median_norm %>% 
  filter(`pos|score` <= ntc_median_pos) %>% 
  select(id, `pos|score`)  %>% 
  pivot_longer(cols = -id,
               names_to = "score_type",
               values_to = "score") %>% 
...
# see which genes are both >= pos and <= neg 
genes_both_pos_neg <- bind_rows(neg_genes, pos_genes) %>% 
  group_by(id) %>% 
  count() %>% 
  filter(n != 1)

bind_rows(neg_genes, pos_genes) %>% 
  mutate(alt_score = case_when(score_type == "neg|score" ~ 0 - score,
                               TRUE ~ score)) %>%
  mutate(color = case_when(str_detect(id, "NTC") ~ "white",
                           score_type == "neg|score" ~ "blue",
                           score_type == "pos|score" ~ "red")) %>%
  ggplot(aes(x = fct_reorder(as.factor(id), alt_score), y = alt_score, fill = color)) +
